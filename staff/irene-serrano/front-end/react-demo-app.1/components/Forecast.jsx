    ...
    render(){
        if(this.state.values && this.state.city){
            ...
            return <div className="forecast-container panel_mini">
                <h3 className="forecast-title vertical-cont">El tiempo en <span className="forecast-title capitalize">{this.state.city}</span></h3> 
                
                {tempValues.map((day)=>{
                    
                    return <Temperature max={day.maxt} min={day.mint} conditions={day.conditions} key={day.datetime} date={day.datetime}/>
                })}
